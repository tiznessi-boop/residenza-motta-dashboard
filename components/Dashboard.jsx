...
const CANCEL={r25:13.6,r26:19.6};
...
if(CANCEL.r26>15)amber.push({title:"Cancellation Rate: "+CANCEL.r26+"% (up from "+CANCEL.r25+"%)",metric:"89 cancellations \u2014 83% from Booking.com",actions:["Review Booking.com cancellation policies","Non-refundable rate options","Analyze cancellation clustering","Expedia: 4/5 bookings cancelled"],impact:"Reducing by 5% retains ~10 bookings"});
amber.push({title:"OTA Dependency: Booking.com at 71%",metric:"255/365 bookings \u2014 high commission exposure (~15%)",actions:["Boost direct bookings on residenzamotta.ch","'Book Direct' best-rate guarantee","Grow Airbnb (currently 8%)","Google Hotel Ads"],impact:"10% shift to direct saves ~"+fmtCHF(TOTAL26.rev*0.10*0.15)+" commissions"});
...
amber.push({title:"Cancellation Clusters: Mar, Jun, Jul = 6 Each",metric:"18 of 89 cancellations in 3 months",actions:["Investigate drivers per month","Stricter Jul policies"],impact:"Half of July's 6 cancellations = significant peak revenue"});
...
const oChart=OTB.filter(m=>m.m>=from&&m.m<=to).map(m=>({name:MONTHS[m.m-1],"OTB 01.07.26":m.r26,"OTB 01.07.25":m.r25}));
...
<div style={{background:"rgba(255,255,255,0.06)",borderRadius:8,padding:"8px 14px",fontSize:12,color:"rgba(255,255,255,0.5)"}}>📅 <span style={{color:"#fff",fontWeight:600}}>July 1, 2026</span><br/>11 units · Locarno · Confirmed + modified only</div>
</div>
{/* TABS */}
...
<KPICard label="OTB vs STLY (01.07)" value={fmtCHF(fOtb.r26)} sub={"OTB 01.07.25: "+fmtCHF(fOtb.r25)} pctLabel={fOtb.r25>0?fmtPct(delta(fOtb.r26,fOtb.r25)):"N/A"} status={cs(fOtb.r26,fOtb.r25)}/>
</div>
...
<strong style={{color:"#a5b4fc"}}>How this works:</strong> 2026 OTB = all confirmed/modified bookings as of <strong style={{color:"#fff"}}>July 1, 2026</strong>. STLY = only 2025 bookings created on or before <strong style={{color:"#fff"}}>July 1, 2025</strong>. Same cutoff date, apples to apples.
</div>
<STitle sub={"OTB as of 01.07.2026 vs OTB as of 01.07.2025"}>{"\ud83d\udcca"} On The Books — {periodLabel}</STitle>
<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:24}}>
<KPICard label="Revenue (OTB 01.07.26)" value={fmtCHF(fOtb.r26)} sub={"OTB 01.07.25: "+fmtCHF(fOtb.r25)} pctLabel={fOtb.r25>0?fmtPct(delta(fOtb.r26,fOtb.r25)):"N/A"} status={cs(fOtb.r26,fOtb.r25)}/>
<KPICard label="Room Nights (OTB 01.07.26)" value={fOtb.rn26} sub={"OTB 01.07.25: "+fOtb.rn25} pctLabel={fOtb.rn25>0?fmtPct(delta(fOtb.rn26,fOtb.rn25)):"N/A"} status={cs(fOtb.rn26,fOtb.rn25)}/>
<KPICard label="ADR (OTB 01.07.26)" value={"CHF "+(fOtb.rn26>0?(fOtb.r26/fOtb.rn26).toFixed(0):"0")} sub={"OTB 01.07.25: CHF "+(fOtb.rn25>0?(fOtb.r25/fOtb.rn25).toFixed(0):"0")} pctLabel={fOtb.rn25>0&&fOtb.rn26>0?fmtPct(delta(fOtb.r26/fOtb.rn26,fOtb.r25/fOtb.rn25)):"N/A"} status={fOtb.rn25>0&&fOtb.rn26>0?cs(fOtb.r26/fOtb.rn26,fOtb.r25/fOtb.rn25):"amber"}/>
<KPICard label="RevPAR (OTB 01.07.26)" value={"CHF "+(f26.arn>0?(fOtb.r26/f26.arn).toFixed(2):"0")} sub={"OTB 01.07.25: CHF "+(f25.arn>0?(fOtb.r25/f25.arn).toFixed(2):"0")} pctLabel={f25.arn>0&&f26.arn>0?fmtPct(delta(fOtb.r26/f26.arn,fOtb.r25/f25.arn)):"N/A"} status={f25.arn>0&&f26.arn>0?cs(fOtb.r26/f26.arn,fOtb.r25/f25.arn):"amber"}/>
<KPICard label="Bookings (OTB 01.07.26)" value={fOtb.bk26} sub={"OTB 01.07.25: "+fOtb.bk25} pctLabel={fOtb.bk25>0?fmtPct(delta(fOtb.bk26,fOtb.bk25)):"N/A"} status={cs(fOtb.bk26,fOtb.bk25)}/>
<KPICard label="Occupancy (OTB 01.07.26)" value={(f26.arn>0?(fOtb.rn26/f26.arn*100).toFixed(1):"0")+"%"} sub={"OTB 01.07.25: "+(f25.arn>0?(fOtb.rn25/f25.arn*100).toFixed(1):"0")+"%"} pctLabel={f25.arn>0&&f26.arn>0?fmtPct(delta(fOtb.rn26/f26.arn,fOtb.rn25/f25.arn)):"N/A"} status={f25.arn>0&&f26.arn>0?cs(fOtb.rn26/f26.arn,fOtb.rn25/f25.arn):"amber"}/>
</div>
<div style={{background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"16px 8px 8px"}}><ResponsiveContainer width="100%" height={320}><ComposedChart data={oChart}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/><XAxis dataKey="name" tick={{fill:"rgba(255,255,255,0.5)",fontSize:11}}/><YAxis tick={{fill:"rgba(255,255,255,0.5)",fontSize:11}} tickFormatter={fmt}/><Tooltip content={<CTip/>}/><Legend wrapperStyle={{fontSize:11}}/><Bar dataKey="OTB 01.07.26" fill="#6366f1" radius={[4,4,0,0]}/><Bar dataKey="OTB 01.07.25" fill="rgba(255,255,255,0.2)" radius={[4,4,0,0]}/></ComposedChart></ResponsiveContainer></div>
<div style={{marginTop:24,overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{borderBottom:"1px solid rgba(255,255,255,0.1)"}}>{["Month","OTB 01.07.26","OTB 01.07.25","\u0394 CHF","\u0394 %","RN 26","RN 25"].map(h=>(<th key={h} style={{padding:"10px 8px",textAlign:"left",color:"rgba(255,255,255,0.5)",fontWeight:600,whiteSpace:"nowrap"}}>{h}</th>))}</tr></thead><tbody>{OTB.filter(m=>m.m>=from&&m.m<=to).map((m)=>{const st=m.r25>0?cs(m.r26,m.r25):(m.r26>0?"green":"amber");return(<tr key={m.m} style={{borderBottom:"1px solid rgba(255,255,255,0.05)"}}><td style={{padding:"10px 8px",fontWeight:600,color:"#fff"}}>{MONTHS[m.m-1]}</td><td style={{padding:"10px 8px",color:"#fff"}}>{fmtCHF(m.r26)}</td><td style={{padding:"10px 8px",color:"rgba(255,255,255,0.5)"}}>{fmtCHF(m.r25)}</td><td style={{padding:"10px 8px",color:sc[st],fontWeight:600}}>{m.d>=0?"+":""}{fmtCHF(m.d)}</td><td style={{padding:"10px 8px",color:sc[st],fontWeight:600}}>{m.r25>0?fmtPct(m.dp):"N/A"}</td><td style={{padding:"10px 8px",color:"#fff"}}>{m.rn26}</td><td style={{padding:"10px 8px",color:"rgba(255,255,255,0.5)"}}>{m.rn25}</td></tr>);})}<tr style={{borderTop:"2px solid rgba(255,255,255,0.15)",fontWeight:700}}><td style={{padding:"10px 8px",color:"#fff",fontWeight:700}}>TOTAL</td><td style={{padding:"10px 8px",color:"#fff",fontWeight:700}}>{fmtCHF(fOtb.r26)}</td><td style={{padding:"10px 8px",color:"#fff",fontWeight:700}}>{fmtCHF(fOtb.r25)}</td><td style={{padding:"10px 8px",color:"#fff",fontWeight:700}} style={{padding:"10px 8px",color:sc[cs(fOtb.r26,fOtb.r25)],fontWeight:700}}>{fOtb.r26-fOtb.r25>=0?"+":""}{fmtCHF(fOtb.r26-fOtb.r25)}</td><td style={{padding:"10px 8px",color:sc[cs(fOtb.r26,fOtb.r25)],fontWeight:700}}>{fOtb.r25>0?fmtPct((fOtb.r26-fOtb.r25)/fOtb.r25*100):"N/A"}</td><td style={{padding:"10px 8px",color:"#fff",fontWeight:700}}>{fOtb.rn26}</td><td style={{padding:"10px 8px",color:"#fff",fontWeight:700}}>{fOtb.rn25}</td></tr></tbody></table></div>
</div>)}
...
{/* FOOTER */}
<div style={{marginTop:48,paddingTop:20,borderTop:"1px solid rgba(255,255,255,0.06)",textAlign:"center"}}><p style={{fontSize:11,color:"rgba(255,255,255,0.25)",margin:0}}>Residenza Motta — July 1, 2026 — 11 Units — Locarno, Ticino</p><p style={{fontSize:10,color:"rgba(255,255,255,0.15)",margin:"6px 0 0"}}>Confirmed + modified only · OTB: 2025 bookings created on or before 01.07.2025 only · Budget from 2026 Business Plan · Direct = Amenitiz + Manual bookings</p></div>
</div>);}
